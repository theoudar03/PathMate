...
const CampusMap = () => {
  ...
  const handleAiSearch = async (queryToSearch) => {
    ...
    setAiIntentMsg(t('aiAnalyzingIntent') || "Gemini AI analyzing navigation intent...");
    try {
      ...
      console.error(err);
      setAiIntentMsg(t('aiNavigationFailed') || "AI Navigation failed.");
    } finally {
  ...
  const getBlockStyle = (category) => {
    if (isDark) {
      switch (category) {
        case 'Sports':
          return { fill: '#1E3F27', stroke: '#4CAF50', text: '#8CE79F', dot: '#8CE79F' };
        case 'Academic':
          return { fill: '#1E293B', stroke: '#3B82F6', text: '#A4C6FF', dot: '#A4C6FF' };
        case 'Services':
        case 'Religious':
          return { fill: '#3E2723', stroke: '#FF9800', text: '#FFB74D', dot: '#FFB74D' };
        case 'Hostel':
          return { fill: '#2E1E3F', stroke: '#8B5CF6', text: '#DEE1F9', dot: '#DEE1F9' };
        case 'Transport':
        case 'Utilities':
        default:
          return { fill: '#252932', stroke: '#78909C', text: '#ECEFF1', dot: '#B0BEC5' };
      }
    }
    switch (category) {
      case 'Sports':
  ...
  return (
    <div className="space-y-6 font-sans text-left max-w-7xl mx-auto py-4 animate-fade-in select-none">
      ...
      <div className="border-b border-surfaceVariant pb-4 flex flex-col md:flex-row items-start md:items-end justify-between gap-4">
        <div>
          <span className="text-xs text-onSurfaceVariant font-bold uppercase tracking-wider">{t('campusPortal') || 'Saranathan Campus Portal'}</span>
          <h1 className="text-3xl font-extrabold text-primary flex items-center gap-2 mt-0.5">
            ...
            {t('campusMapHeader') || 'Campus Map & Live Navigation'}
          </h1>
          <p className="text-sm text-onSurfaceVariant mt-1">
            {t('campusMapDesc') || 'Pick your destination building, track live GPS walking directions, and explore 18+ SCE landmarks.'}
          </p>
        </div>
        ...
        <div className="flex border border-outline/30 rounded-full p-1 bg-white shadow-xs w-full sm:w-auto">
          ...
          >
            ...
            <span>🛰️ {t('satelliteView') || 'Satellite View'}</span>
          </button>
          ...
          >
            ...
            <span>🗺️ {t('layoutMap') || '2D Layout Map'}</span>
          </button>
        </div>
      </div>
        ...
        <div className="flex flex-col md:flex-row items-center gap-3">
          <div className="flex items-center gap-3 min-w-0 flex-1 w-full bg-surfaceContainerLow border border-outline/25 rounded-full px-4 py-1.5 focus-within:ring-2 focus-within:ring-primary">
            ...
            <input
              ...
              placeholder={t('mapSearchPlaceholder') || "Ask AI to navigate anywhere on campus... (e.g. 'take me to cse department')"}
          ...
          >
            ...
            <span>{isAiSearching ? (t('analyzing') || 'Analyzing...') : (t('askAiNavigation') || 'Ask AI Navigation')}</span>
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div className="lg:col-span-2">
          {viewMode === 'satellite' ? (
            ...
            <div className="relative w-full h-[520px] sm:h-[820px] bg-surface border border-outline/30 rounded-3xl overflow-hidden shadow-elevation2">
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    {/* Ground Background */}
                    <rect x="0" y="0" width="900" height="980" fill={isDark ? "#181A1F" : "#F4F3F0"} />

                    {/* Grassy Areas & Dirt Tracks */}
                    {/* Main Cricket Ground Ring Track */}
                    <ellipse cx="240" cy="100" rx="205" ry="70" fill="none" stroke={isDark ? "#2A2F3D" : "#E0DDD5"} strokeWidth="6" opacity="0.5" />
                    {/* Cricket Ground 2 turf */}
                    <rect x="30" y="355" width="310" height="285" rx="20" fill={isDark ? "#1A2E22" : "#E2F0D9"} stroke={isDark ? "#2D5438" : "#C3D9B5"} strokeWidth="1" />
                    <rect x="50" y="375" width="270" height="245" rx="10" fill="none" stroke="#FFFFFF" strokeDasharray="6 4" strokeWidth="1" opacity="0.15" />
                    {/* Cricket Ground 1 turf */}
                    <rect x="30" y="705" width="310" height="220" rx="20" fill={isDark ? "#1A2E22" : "#E2F0D9"} stroke={isDark ? "#2D5438" : "#C3D9B5"} strokeWidth="1" />
                    
                    <g className="roads" opacity="1">
                      {/* Main Roads (Google White Asphalt Style) */}
                      <rect x="0" y="175" width="900" height="26" fill={isDark ? "#2A2F3D" : "#FFFFFF"} stroke={isDark ? "#3A3F4D" : "#E8EAED"} strokeWidth="1.5" />
                      <line x1="0" y1="188" x2="900" y2="188" stroke={isDark ? "#4A5568" : "#FEEFC3"} strokeWidth="1.5" strokeDasharray="6 4" />
                      
                      <rect x="435" y="175" width="30" height="795" fill={isDark ? "#2A2F3D" : "#FFFFFF"} stroke={isDark ? "#3A3F4D" : "#E8EAED"} strokeWidth="1.5" />
                      <line x1="450" y1="175" x2="450" y2="970" stroke={isDark ? "#4A5568" : "#FEEFC3"} strokeWidth="1.5" strokeDasharray="6 4" />
                      
                      <rect x="0" y="660" width="900" height="26" fill={isDark ? "#2A2F3D" : "#FFFFFF"} stroke={isDark ? "#3A3F4D" : "#E8EAED"} strokeWidth="1.5" />
                      <line x1="0" y1="673" x2="900" y2="673" stroke={isDark ? "#4A5568" : "#FEEFC3"} strokeWidth="1.5" strokeDasharray="6 4" />

                      {/* center vertical path from KS block to mechanical workshop */}
                      <rect x="656" y="175" width="14" height="485" fill={isDark ? "#2A2F3D" : "#FFFFFF"} stroke={isDark ? "#3A3F4D" : "#E8EAED"} strokeWidth="1" />
                      <line x1="663" y1="175" x2="663" y2="660" stroke={isDark ? "#3A3F4D" : "#E8EAED"} strokeWidth="1" strokeDasharray="3 3" />

                      {/* horizontal path between Mechanical Lab and KS Block */}
                      <rect x="465" y="522" width="205" height="11" fill={isDark ? "#2A2F3D" : "#FFFFFF"} stroke={isDark ? "#3A3F4D" : "#E8EAED"} strokeWidth="1" />
                      <line x1="465" y1="527.5" x2="670" y2="527.5" stroke={isDark ? "#3A3F4D" : "#E8EAED"} strokeWidth="1" strokeDasharray="3 3" />

                      {/* New Right Column Horizontal Roads */}
                      {/* 1. Between Bus Boarding Point and Staff Parking Lot */}
                      <rect x="656" y="260" width="244" height="15" fill={isDark ? "#2A2F3D" : "#FFFFFF"} stroke={isDark ? "#3A3F4D" : "#E8EAED"} strokeWidth="1" />
                      <line x1="656" y1="267.5" x2="900" y2="267.5" stroke={isDark ? "#3A3F4D" : "#E8EAED"} strokeWidth="1" strokeDasharray="3 3" />

                      {/* 2. Between Staff Parking Lot and BD Block */}
                      <rect x="656" y="330" width="244" height="15" fill={isDark ? "#2A2F3D" : "#FFFFFF"} stroke={isDark ? "#3A3F4D" : "#E8EAED"} strokeWidth="1" />
                      <line x1="656" y1="337.5" x2="900" y2="337.5" stroke={isDark ? "#3A3F4D" : "#E8EAED"} strokeWidth="1" strokeDasharray="3 3" />
                    </g>
                    ...
                    {CAMPUS_MAP_DATA.map(block => {
                      ...
                      let strokeCol = isSelected ? (isDark ? '#60A5FA' : '#2563EB') : style.stroke;
                      let strokeW = isSelected ? 4 : 1.8;

                      if (block.category === 'Sports') {
                        if (block.id === 'volleyball-court') {
                          fillCol = isDark ? '#3D2F23' : '#EAD6BD'; // warm beach sand court
                        } else if (block.id === 'basketball-court') {
                          fillCol = isDark ? '#1F3443' : '#4A7F9D'; // synthetic acrylic basketball blue
                        } else {
                          fillCol = isDark ? '#1E3620' : '#81B87D'; // professional turf green
                        }
                      } else if (block.category === 'Academic') {
                        fillCol = isDark ? '#2C3545' : '#FFFFFF'; // concrete roof
                      } else if (block.category === 'Hostel') {
                        fillCol = isDark ? '#2E2838' : '#F8FAFC'; // hostel roof
                      } else if (isParking) {
                        fillCol = isDark ? '#1C1F26' : '#4F5866'; // slate charcoal asphalt
                      }
                        ...
                        return (
                          ...
                          >
                            ...
                            <text
                              ...
                              textAnchor="middle"
                              dominantBaseline="central"
                              fill={isDark ? "#8D9199" : "#5F6368"}
                              fontSize="8.5"
                              fontWeight="800"
                              className="pointer-events-none uppercase tracking-wider select-none"
                            >
                              {t('mapCategory_' + block.category) || block.category}
                            </text>
                          </g>
                        );
                      } else {
                ...
                >
                  <div className="flex justify-between items-start border-b border-slate-100 pb-3 mb-3">
                    ...
                    <button 
                      ...
                      aria-label={t('closeText') || "Close details"}
                    >
                  ...
                  <p className="text-xs text-slate-600 font-semibold leading-relaxed mb-3.5">
                    {blockDetails.description ? <TranslateText text={blockDetails.description} /> : 'Welcome to this campus building location. Utilize navigation to locate live steps from any starting gate.'}
                  </p>
                  
                  {blockDetails.departments && blockDetails.departments.length > 0 && (
                    <div className="mb-4">
                      <span className="text-[9.5px] font-black text-slate-400 uppercase tracking-wider block mb-1">{t('departmentsLabel') || 'Departments'}</span>
                      <div className="flex flex-wrap gap-1">
                        {blockDetails.departments.map(d => (
                          <span key={d} className="px-2.5 py-0.5 bg-slate-50 border border-slate-200/40 rounded text-[9.5px] font-bold text-slate-600">
